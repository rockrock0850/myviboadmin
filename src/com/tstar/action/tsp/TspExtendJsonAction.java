...
@Component
@Scope("prototype")
public class TspExtendJsonAction implements Action, ServletRequestAware{
	private final Logger logger = Logger.getLogger(TspExtendJsonAction.class);
    ...
    public String contactUsReplyRead(){
		return actionRun(Thread.currentThread().getStackTrace()[1].getMethodName());
    }
    
    public String contactUsIssueAddAgain(){
		return actionRun(Thread.currentThread().getStackTrace()[1].getMethodName());
    }
    ...
    public String depositBankInfo(){
		return actionRun(Thread.currentThread().getStackTrace()[1].getMethodName());
    }
    
    public String cwsOptions(){
		return actionRun(Thread.currentThread().getStackTrace()[1].getMethodName());
    }
    
    public String cwsContactUS(){
		return actionRun(Thread.currentThread().getStackTrace()[1].getMethodName());
    }
    
    public String contactUsIssueQuery(){
		return actionRun(Thread.currentThread().getStackTrace()[1].getMethodName());
    }
