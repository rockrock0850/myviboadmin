...
/*********************************************************************************************************************/
//檢查用戶門號密碼是否正確
public String DoAuthCheck_old(String sRequestXML){	//不使用
	String		sRequestId		= "";
	String		sMSISDN			= "";
	String		sPassword		= "";
...
public String DoAuthCheck(String sRequestXML){
	/**********************有資料了，開始做事吧*********************************************/
	...
	//if (sMSISDN.equals("0986441201")) return "<BscXmlApi><DoAuthCheckRes><RequestId>" + sRequestId + "</RequestId><ResultCode>99002</ResultCode><ResultText>本APP已終止服務，請至Google Play或App Store下載新的APP!</ResultText><AppURL><![CDATA[http://www.google.com/]]></AppURL><ContractId></ContractId><CustomerId></CustomerId></DoAuthCheckRes></BscXmlApi>";
	//if (sMSISDN.equals("0986441202")) return "<BscXmlApi><DoAuthCheckRes><RequestId>" + sRequestId + "</RequestId><ResultCode>99003</ResultCode><ResultText>本服務僅提供威寶電信3G月租型用戶使用，如有疑問請洽客服0800-661234</ResultText><ContractId></ContractId><CustomerId></CustomerId></DoAuthCheckRes></BscXmlApi>";
	...
	/************************以下是每個BSC API各自的參數************************/
	sXML = sXML + "<MSISDN>" + sMSISDN + "</MSISDN>";
	...
	try{
		...
		if (htResponse.get("ResultCode").toString().equals(gcResultCodeSuccess)){	//正常回覆，顯示回覆資料內容
			sResponse = htResponse.get("Response").toString();
			sResponse = htResponse.get("Response").toString();
			sOperatorId = GetXMLSingleValue(sResponse, "OperatorId");
			if (notEmpty(sOperatorId) && !sOperatorId.equals("01")){
				sResponse	= "<BscXmlApi><DoAuthCheckRes><RequestId>" + sRequestId + "</RequestId><ResultCode>99003</ResultCode><ResultText>本服務僅提供威寶電信3G月租型用戶使用，如有疑問請洽客服0800-661234</ResultText><ContractId></ContractId><CustomerId></CustomerId></DoAuthCheckRes></BscXmlApi>";
			}else{
				sContractId = GetXMLSingleValue(sResponse, "ContractId");
				sCustomerId = GetXMLSingleValue(sResponse, "CustomerId");
				if (notEmpty(sContractId) && notEmpty(sCustomerId)){
					sResponse	= "<BscXmlApi><DoAuthCheckRes><RequestId>" + sRequestId + "</RequestId><ResultCode>00000</ResultCode><ResultText>Success</ResultText><ContractId>" + sContractId + "</ContractId><CustomerId>" + sCustomerId + "</CustomerId></DoAuthCheckRes></BscXmlApi>";
				}else{
					sResponse	= "<BscXmlApi><DoAuthCheckRes><RequestId>" + sRequestId + "</RequestId><ResultCode>00006</ResultCode><ResultText>找不到符合條件的資料</ResultText><ContractId></ContractId><CustomerId></CustomerId></DoAuthCheckRes></BscXmlApi>";
			...
			sResponse	= "<BscXmlApi><DoAuthCheckRes><RequestId>" + sRequestId + "</RequestId><ResultCode>" + gcResultCodeUnknownError + "</ResultCode><ResultText>" + gcResultTextUnknownError + "</ResultText><ContractId></ContractId><CustomerId></CustomerId></DoAuthCheckRes></BscXmlApi>";
