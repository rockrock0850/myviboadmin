...
// 						alert(key);
		...
		function init(){
			try {
				...
				if("" != project_name1){
					...
					var cpt_compare = parseInt(cal_amount2) + parseInt(single_phone_tstar);
					...
					case ''://我方沒手機價
						if(single_phone_tstar == ''){//兩方都沒有手機價才隱藏專案價欄位
				...
				if("" != project_name2){
					...
					checkValue($("#phone_discount2"), single_phone_tstar, parseInt(single_phone_tstar) - parseInt(single_phone_chosen), "-", "$", "");//對方攜碼手機折扣
					checkValue($("#cell_plus_total2"), single_phone_tstar, parseInt(cal_amount2) + parseInt(single_phone_tstar), "-", "$", "");//手機+合約期間帳單總支付金額
					...
					checkValue($("#sms_off_net2"), sms_off_net2, sms_off_net2, "-", "", "");//網外簡訊	
		 			...
		 			$('#no_fee_single').attr('rowspan', $('tr').size() - 1);
				}	
			} catch (e) {
				alert(e);
			}	
	...
	<div class="n_sec_content">
		<div class="n_result">
			...
			<div class="n_result_item">推薦您台灣之星最適專案:<br/>
			...
			<div class="n_result_item" hidden="true">
			...
			<tr>
				<th align="center">類別</th>
				<td align="center">單門號+空機</td>
				<td align="center">手機專案</td>
			</tr>
			<tr>
				...
				<c:if test='${empty proJsonTstar.getSingle_project_name()}'>
					<td id="no_fee_single">目前無此資費</td>
				</c:if>
				<c:if test='${not empty proJsonTstar.getSingle_project_name()}'>
					<td align="center">${proJsonTstar.getSingle_project_name()}</td>
				</c:if>
			</tr>
			<tr>
			...
			<c:forEach items="${ProjectDeviceDtoOrder}" var="project">
			    <tr id="tr_cell_phone_price">
					...
					<c:if test='${not empty proJsonTstar.getSingle_project_name()}'>
						<c:if test="${not empty project.singlePhoneTstar}">
							<td align="center">$<fmt:formatNumber type="currency" currencySymbol="$" pattern="#,##0;" value="${project.singlePhoneTstar}" /></td>
						</c:if>
