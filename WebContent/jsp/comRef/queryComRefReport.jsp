...
<script>
	...
	function submitForm(){
		if($("#createtimestart").val() == ''){
			alert("請輸入建立時間(起)");
		}else if($("#createtimeend").val() == ''){
			alert("請輸入建立時間(迄)");
		}else if($("#createtimestart").val()>$("#createtimeend").val()){
			alert("建立時間(起)不能大於建立時間(迄)");
		}else{
			$('#createtimestart').attr('value', $('#createtimestart').val());
			$('#createtimestart').attr('value', $('#createtimeend').val());
			$("#cf_form").submit();	
...
<div id="wrapper">
	...
	<form id='cf_form' action="comRefReportDownloadExcel.action" method='post'>
		<table class="filtered_table" cellspacing="10">
			<tr><td colspan="4" align="center"><h2>請輸入搜尋條件</h2></td></tr>
			<tr>
				<td>建立時間起</td>
				<td><input type="text" id="createtimestart" name="createtimestart"></td>
				<td>建立時間迄</td>
				<td><input type="text" id="createtimeend" name="createtimeend"></td>			
			</tr>
		<tr>
			<td colspan="4" align="center"><input class="submit btn" type="button" onclick='submitForm()' value="下載" />
